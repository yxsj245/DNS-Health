...
// GetTaskLatency 获取探测任务的延迟数据
// GET /api/tasks/:id/latency?ip=xxx&start_time=xxx&end_time=xxx
// 按 probed_at 升序返回指定任务、IP、时间范围内的延迟数据
// 验证需求：1.1, 1.3, 1.4, 1.5, 1.6
func (h *StatusHandler) GetTaskLatency(c *gin.Context) {
	// 解析任务 ID
	idStr := c.Param("id")
